...
document.addEventListener("DOMContentLoaded", () => {
    ...
    formMetricas.addEventListener("submit", (e) => {
        ...
        nyvoraAddMetric({
            ...
            bodyFatPercentage:
                parseFloat(document.getElementById("grasa").value) || null,
            heartRate:
                parseInt(document.getElementById("fc").value, 10) || null,
            sleepHours:
                parseFloat(document.getElementById("sueno").value) || null,
            steps:
                parseInt(document.getElementById("pasos").value, 10) || null
        });
    ...
    function renderMetricas(lista = nyvoraGetMetrics()) {
        ...
        lista.forEach((metrica) => {
            ...
            fila.innerHTML = `
                <td>${nyvoraFormatDate(metrica.measurementDate)}</td>
                <td>
                    ${
                        paciente
                            ? nyvoraEscapeHtml(paciente.fullName)
                ...
                <td>${metrica.steps?.toLocaleString() ?? "—"}</td>
            `;
    ...
    document.getElementById("btn-filtrar").addEventListener("click", () => {
        ...
        if (desde) {
            metricas = metricas.filter(
                (metrica) => metrica.measurementDate.slice(0, 10) >= desde
            );
        }

        if (hasta) {
            metricas = metricas.filter(
                (metrica) => metrica.measurementDate.slice(0, 10) <= hasta
            );
        }
    ...
    window.addEventListener("nyvora:data-changed", () => renderMetricas());
